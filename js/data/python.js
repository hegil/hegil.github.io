...
COURSES.python = {
    ...
    units: [{
      ...
      ready: true,
      intro: true,
      summary: '파이썬이 무엇이고, 어디에 쓰이고, 왜 첫 언어로 많이 추천되는지 알아봐요.',
      blocks: [
        {
          h: '파이썬은 어떤 언어인가요?',
          html: `<p>파이썬은 1991년 귀도 반 로섬이 만든 프로그래밍 언어예요. 문법이 간결하고 사람이 읽는 글과 비슷해서, "실행 가능한 의사코드"라고 불릴 정도로 쉬운 편이에요. 중괄호 대신 들여쓰기로 코드 블록을 구분하는 것도 특징이에요.</p>`
        },
        {
          h: '어디에 쓰이나요?',
          html: `<p>데이터 분석과 인공지능·머신러닝(NumPy, Pandas, TensorFlow 등), 웹 백엔드(Django, Flask), 업무 자동화 스크립트, 교육용 프로그래밍 등 정말 다양한 곳에 쓰여요. 요즘 가장 널리 쓰이는 언어 중 하나예요.</p>`
        },
        {
          h: '왜 배우면 좋을까요?',
          html: `<p>문법 자체의 부담이 적어서 "프로그래밍적으로 생각하는 법"에 집중할 수 있어요. 그래서 완전 처음 프로그래밍을 배우는 사람에게 가장 많이 추천되는 언어예요. 여기서 배운 개념(변수, 조건문, 반복문, 함수 등)은 나중에 다른 언어를 배울 때도 그대로 도움이 돼요.</p>`,
