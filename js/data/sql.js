...
COURSES.sql = {
    ...
    units: [{
      ...
      ready: true,
      intro: true,
      summary: 'SQL이 무엇이고, 어디에 쓰이고, 왜 배우면 좋은지 알아봐요.',
      blocks: [
        {
          h: 'SQL은 어떤 언어인가요?',
          html: `<p>SQL은 데이터베이스에 저장된 데이터를 조회하고, 추가하고, 수정하고, 삭제하기 위한 전용 언어예요. 1970년대에 개발되어 지금까지도 거의 모든 데이터베이스의 표준 언어로 쓰이고 있어요.</p>`
        },
        {
          h: '어디에 쓰이나요?',
          html: `<p>웹사이트, 앱, 회사 내부 시스템 등 데이터를 저장하는 거의 모든 서비스의 뒤편에서 SQL이 동작해요. 회원 정보, 주문 내역, 게시글 등 우리가 보는 대부분의 데이터가 SQL로 관리돼요.</p>`
        },
        {
          h: '왜 배우면 좋을까요?',
          html: `<p>다른 프로그래밍 언어와는 문법이 아예 달라요 — 반복문이나 조건문으로 "어떻게" 할지 적는 대신, "무엇을 원하는지"만 선언하면 데이터베이스가 알아서 찾아줘요. 처음엔 낯설지만, 개발자든 데이터 분석가든 데이터를 다루는 모든 직군에게 필수인 언어예요.</p>`,
