...
COURSES.javascript = {
    ...
    units: [{
      id: 'intro',
      title: '자바스크립트는 어떤 언어인가요?',
      ready: true,
      intro: true,
      summary: '자바스크립트가 무엇이고, 어디에 쓰이고, 왜 웹 개발에 필수인지 알아봐요.',
      blocks: [
        {
          h: '자바스크립트는 어떤 언어인가요?',
          html: `<p>자바스크립트는 1995년, 웹 브라우저 안에서 실행되는 유일한 프로그래밍 언어로 태어났어요. 지금은 Node.js 덕분에 서버에서도 실행할 수 있게 됐어요.</p>`
        },
        {
          h: '어디에 쓰이나요?',
          html: `<p>버튼 클릭에 반응하고, 화면 내용을 실시간으로 바꾸고, 서버와 데이터를 주고받는 등 웹사이트의 모든 "상호작용"을 담당해요. React, Vue 같은 인기 프레임워크들도 전부 자바스크립트를 기반으로 해요.</p>`
        },
        {
          h: '왜 배우면 좋을까요?',
          html: `<p>웹 개발을 하려면 사실상 필수인 언어예요. 브라우저만 있으면 바로 실행해볼 수 있어서 결과를 눈으로 빠르게 확인할 수 있고, 비동기 처리·이벤트 기반 프로그래밍 같은 현대적인 개념을 배우기에도 좋아요.</p>`,
